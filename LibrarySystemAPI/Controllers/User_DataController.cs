...
namespace LibrarySystemAPI.Controllers
{
    ...
    public class User_DataController : ControllerBase
    {
        ...
        public IActionResult GetUserdata(string id)
        {
            try
            {
                var users = _context.User_Data.Where(s => s.UserId == id).ToList();
                if (users.Count() == 0)
                {
                    throw new Exception("No user Available");
                }
        ...
        public IActionResult GetForLogin(string name, string password)
        {
            try
            {
                ...
                var users = _context.User_Data.FirstOrDefault(x => x.UserEmail == name);
                if(users == null)
                {
                    throw new Exception("User Name and Password does not match");
                }
                if(users.UserPass != password)
                {
                    throw new Exception("User Name and Password does not match");
                }
                if(users.UserAuth == "1")
                ...
                return Ok(new {status="Success",message="Login Successful :)", data=users.UserId});
            }
            catch (Exception ex)
            {
                return Ok(new {status=status,message=ex.Message});
        ...
        public IActionResult Post(User_Data model)
        {
            try
            {
                ...
                var userN = _context.User_Data.Where(s => s.UserEmail == model.UserEmail).ToList();
                if (user.Count >0)
                {
