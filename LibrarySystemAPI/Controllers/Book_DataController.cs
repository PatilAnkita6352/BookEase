...
namespace LibrarySystemAPI.Controllers
{
    ...
    public class Book_DataController : ControllerBase
    {
        ...
        public IActionResult GetGenere()
        {
            try
            {
                var Genere= (from m in _context.Book_Data
                            select m.BookGenre).Distinct().ToList();
                return Ok(new { status = "Success", data = Genere });
            }
            catch (Exception ex)
            {
                message = ex.Message;
        ...
        [HttpGet("{title}")]
        public IActionResult GetBookAutocomplete(string title, int fla=1 )
        {
            try
            {
                if( fla == 1)
                {
                    var book = _context.Book_Data.Where(s => s.BookTitle.Contains(title) && s.BookStatus != "Removed").ToList();
                    if (book.Count == 0)
                    {
        ...
        [HttpGet]
        public IActionResult GetBookSearch(string title, string genre)
        {
            try
            {
                IQueryable<Book_Data> query = _context.Book_Data;

                if (title!="None" && genre != "None")
                {
                    query = query.Where(b => b.BookTitle.Contains(title) && b.BookGenre.Contains(genre));
                }
                else if (title!="None")
                {
                ...
                {
                    query = query.Where(b => b.BookGenre.Contains(genre));
                }
                else
                {
                    // If both title and genre are null or empty, return bad request
                    throw new Exception("Please provide either title, genre, or both for search.");
                }
        ...
        public IActionResult AddBookStock(string Book, int cnt)
        {
            try
            {
                var isDect = _context.Book_Data.Find(Book);
                if(isDect.BookStatus== "Removed")
                {
                    throw new Exception("Book is Deactivate");
                }
        ...
        public IActionResult DeactivateBook(string id) {
            try
            {
                ...
                if (book.BookStatus == "Removed")
                {
                    throw new Exception("Book is Already Deactivated");
                }
        ...
        public IActionResult EditBook(Book_Data model)
        {
            try
            {
                ...
                Console.WriteLine(model.BookTitle);
                _context.SaveChanges();
                status = "Success";
